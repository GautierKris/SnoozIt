...
use Snoozit\PlatformBundle\Entity\UserProfileCommentToComment;
...
class UserProfileCommentToCommentHandler
{
    private $form;
    ...
    private function injectData()
    {
        ...
        $comment = $this->requestStack->getCurrentRequest()->attributes->get('comment');


        $entity = $this->entityManager->getRepository('SnoozitPlatformBundle:UserProfileComment')->find($comment->getId());
        if(!$entity){
            throw new EntityNotFoundException("Le commentaire initiale n'existe pas.");
        }

            $userProfileComment = new UserProfileCommentToComment();
            $userProfileComment->setUserProfileComment($entity);
