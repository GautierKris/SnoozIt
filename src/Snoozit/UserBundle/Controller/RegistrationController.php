...
class RegistrationController extends Controller
{
    public function registerAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            // Ajout pour SnoozIt
            $avatar_default = $this->container->get('doctrine')->getManager()->getRepository('SnoozitUserBundle:Avatar')->find(1);
            $user->setAvatar($avatar_default);

            $userManager->updateUser($user);

            // Logs SnoozIt
            $dispatcher->dispatch(SkuagEvents::ON_REGISTRATION_SUCCESS, new RegistrationSuccess($user));

            $notification = new Notification();
            $notification->setUser($user);

            if (null === $response = $event->getResponse()) {
                $url = $this->generateUrl('snoozit_dashboard_homepage');
