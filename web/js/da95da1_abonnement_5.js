$(document).ready(function(){
    ...
    // Gestions des notes des membres
    $(function (){

        var container = $('#rating');

        $('#rating .ratingvalue').click( function() {

            var note = $(this).attr("rel");
            var userId = container.attr("rel")

            $.ajax({
                type: "POST",
                url: Routing.generate('snoozit_evaluation_system', { 'id': userId , 'note': note }),
                beforeSend: function(){
                    console.log("Envoi de l'évaluation en cours...");
                },
                success: function(data){
                    container.hide();
                    console.log(data);
                    $('.modal-body p').replaceWith("Merci, votre évaluation à bien été prise en compte.");
